...
fun DateTimePickerSheet(
    ...
    ModalBottomSheet(onDismissRequest = onDismiss) {
        Column(
            ...
            Text(
                text = stringResource(R.string.capture_datetime_picker_title),
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                TextButton(onClick = { showingDate = !showingDate }) {
                    Text(
                        if (showingDate) {
                            stringResource(R.string.capture_datetime_to_time)
                        } else {
                            stringResource(R.string.capture_datetime_to_date)
                        },
                    )
                }
                Spacer(Modifier.width(8.dp))
                TextButton(onClick = onDismiss) {
                    Text(stringResource(R.string.capture_datetime_cancel))
                }
                Spacer(Modifier.width(8.dp))
                Button(
                    onClick = {
